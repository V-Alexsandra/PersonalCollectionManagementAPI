...
namespace PersonalCollectionManagement.Business.Validators
{
    public class LoginUserDtoValidator<T> : AbstractValidator<T> where T : LoginUserDto
    {
        public LoginUserDtoValidator()
        {
            ...
            RuleFor(l => l.Password)
                .NotEmpty()
                .Length(8, 50)
                .Matches("^(?=.*[A-Za-z])(?=.*\\d)(?=.*[@$!%*#?&{}\\[\\]()amp;-])[A-Za-z\\d@$!%*#?&{}\\[\\]()amp;-]{8,}$").WithMessage("The entered password must have minimum 8 characters at least 1 alphabet, 1 number and 1 special Character.");
        }
    }
}
